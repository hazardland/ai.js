...
/*
    ...
    INPUT
    -------
    Currently input is any number

    BIAS
    --------
    Input and hidden layers have bias
    Bias values are randomed
    Initial bias value is 1

    WEIGHTS
    ---------
    Weights are random between -1 and 1
    Weight mutation happens with random number between -rate and +rate
    Weights can be any amount after many mutations they have no ranges
...
Neuron.prototype.result = function result (layer)
{
    ...
    for (let neuron=0;neuron<this.weights.length;neuron++)
    {
    ...
    //this.value = 1/(1+Math.pow(Math.E, -this.value)); //SIGMOID
    this.value = this.value<0?0:this.value; //RELU
}
...
Network.prototype.mutate = function (rate)
{
    ...
    //new document says bias value should not be randomized
    //randomize first layer bias value
    //clone.layers[0][clone.layers[0].length-1].value = this.layers[0][this.layers[0].length-1].value + this.random(rate);
...
{
    //i think bias value is not randomized
    // if (this.bias)
    // {
    //     this.value = parent.value + this.random(rate);
    // }
    //for every weight we need to create new weight
    for (let i=0;i<this.weights.length;i++)
    {
        this.weights[i] = parent.weights[i] + this.random(rate);
        // if (this.weights[i]>=2)
        // {
        //     this.weights[i] = 2;
        // }
        // else if (this.weights[i]<=-2)
        // {
        //     this.weights[i] = -2;
        // }
    }
}
Neuron.prototype.random = function (max)
{
    return Math.random() * max * (Math.ceil(Math.random()*2)==1?1:-1);
}
Network.prototype.random = Neuron.prototype.random;
