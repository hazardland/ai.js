/*
    Added 2 step memory to creature brain and productivity increased on 8x8 square from 33 square travel to 42 (by 9 square)
    Relu is faster in training than sigmoid?
    But there is some point from where creatures do not envolve more
...
World.prototype.populate = function(brain)
{
    ...
    let point = this.place();
    // point.x = 10;
    // point.y = 10;
    this.creatures = [];
    //for (let i=0; i<this.trained?1:this.config.creature.count; i++)
    for (let i=0; i<(this.trained?1:this.config.creature.count); i++)
    {
        // if (i==0)
        // {
        ...
        //     point.y = 10;
        // }
        this.creatures.push(new Creature(this,i,point.x,point.y,brain?(!this.trained?brain.mutate(this.config.brain.change):brain):null));
    }
...
World.prototype.run = function (view)
{
    ...
    if (alive==0 && !this.hero)
    {
        ...
        if (hero)
        {
            hero.hero = true;
            this.hero = true;
            if (hero.curiosity>=this.config.goal)
            {
                this.trained = true;
            }
            this.save(hero.brain, this.generation, hero.curiosity);
            if (browser())
            {
...
World.prototype.save = function (brain, generation, score)
{
    //console.log("saved brain "+brain.layers[1][17].weights[13]);
    localStorage.setItem (this.config.name, JSON.stringify(brain));
    localStorage.setItem (this.config.name+".stats", JSON.stringify({generation:generation,score:score}));
}
...
function Wall(world,index,x,y)
{
    this.index = index;
    this.world = world;
    this.type = this.world.config.wall.type;
    this.x = x;
    this.y = y;
    this.alocate();
}
...
function Food(world,index,x,y)
{
    ...
    this.index = index;
    this.world = world;
    this.type = this.world.config.food.type;
    this.x = x;
    this.y = y;
...
function Creature (world,index,x,y,brain)
{
    ...
    this.index = index;
    this.world = world;
    this.type = this.world.config.creature.type;
    this.x = x;
    this.y = y;
...
Creature.prototype.draw = function (view)
{
    ...
    if (this.dinners || this.curiosity)
    {
        ...
        //view.fillText(this.x+","+this.y, this.x*this.world.scale, this.y*this.world.scale);
    }
}
Creature.prototype.scan = function (x,y)
{
    if (this.world.config.borders && (this.x+x<0 || this.x+x>=this.world.width || this.y+y<0 || this.y+y>=this.world.height))
    {
        return this.world.config.wall.type;
    }
    x = this.xx (this.x+x);
    y = this.yy (this.y+y);
    //console.log ("scan at "+x+","+y);
    if (this.world.map[x][y]!=0)
    {
        if (this.world.map[x][y].type==this.world.config.food.type)
        {
...
Creature.prototype.distance = function (x,y,type)
{
    // if (x!=0)
    // {
    //     this
    ...
    // }
    //
    let limit = this.world.config.creature.vision;
    ...
    while (limit>0)
    {
        left = left + x;
        top = top + y;
        if (this.scan(left,top)==type)
        {
            //console.log ("scan "+x+","+y+" --> "+type+" distance "+distance);
            return this.world.config.creature.vision-distance;
        }
        //console.log ("scan "+x+","+y+" --> "+0+" distance "+distance);
        distance++;
        limit--;
    }
    return 0;
...
Creature.prototype.vision = function ()
{
    ...
    //console.log ("we are at "+this.x+","+this.y);
    result = [
         0,0,0,0, //memory of prev 2 moves in x1,y1,x2,y2
         //distance to wall
         this.distance(-1,0,this.world.config.wall.type),
         this.distance(1,0,this.world.config.wall.type),
         this.distance(0,-1,this.world.config.wall.type),
         this.distance(0,1,this.world.config.wall.type),
         //distance to wall
         this.distance(-1,0,this.world.config.food.type),
         this.distance(1,0,this.world.config.food.type),
         this.distance(0,-1,this.world.config.food.type),
         this.distance(0,1,this.world.config.food.type),
    ];
...
Creature.prototype.move = function()
{
    ...
    vision = this.vision();
    vision[0] = this.memory.x[0];
    vision[1] = this.memory.y[0];
    vision[2] = this.memory.x[1];
    vision[3] = this.memory.y[1];
    //let result = [Math.random(),Math.random(),Math.random(),Math.random(),Math.random()];
    let result = this.brain.result(vision);

    if (this.index===0 && this.world.debug)
    {
        this.world.debug(this.brain);
    }
    ...
    {
        this.memory.x[1] = this.memory.x[0];
        this.memory.y[1] = this.memory.y[0];
        this.memory.x[0] = x;
        this.memory.y[0] = y;
...
Creature.prototype.wall = function (x,y)
{
    ...
    if (this.world.config.borders && (x<0 || x>=this.world.width || y<0 || y>=this.world.height))
    {
        return true;
    }
    ...
    x = this.xx(x);
    y = this.yy(y);
    if (this.world.map[x][y]!=0 && this.world.map[x][y].type==this.world.config.wall.type)
    {
        return true;
    }
    return false;
...
{
    if (typeof this.world.map[x][y]!=0 && this.world.map[x][y]!=0 && this.world.map[x][y].type==this.world.config.food.type && !this.eaten[this.world.map[x][y].index])
    {
        return true;
    }
    return false;
